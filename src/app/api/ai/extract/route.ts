...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: edgeResult, error: edgeError } = await supabaseServer.functions.invoke('gemini/extract', {
      body,
    });

    if (edgeError) {
      console.error('Edge Function Extract Error:', edgeError);
      
      // Parse custom daily limit messages or proxy errors correctly
      let errorMsg = 'Failed to extract knowledge';
